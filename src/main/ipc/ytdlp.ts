...
export function registerYtdlpIpc() {
  ...
  ipcMain.handle('library:startUrlImport', async (_e, args: { url: string; titleOverride?: string }) => {
    ...
    // Fire-and-forget the long-running work; report results via events.
    (async () => {
      let tempDir: string | null = null;
      try {
        broadcast('ytdlp:progress', { requestId, phase: 'downloading', message: 'Fetching metadata…' });
        const probe = await ytDlpJson(args.url);
        const title = args.titleOverride?.trim() || probe.title;

        tempDir = await mkdtemp(join(tmpdir(), 'vsw-yt-'));
        const downloadedPath = await downloadVideo(args.url, tempDir, requestId);

        const entry = inflight.get(requestId);
        if (entry?.canceled) throw new Error('Canceled');

        broadcast('ytdlp:progress', { requestId, phase: 'merging', message: 'Finalizing…' });
        ...
        broadcast('ytdlp:progress', { requestId, phase: 'done', message: 'Import complete' });
        broadcast('ytdlp:done', { requestId, meta });
      } catch (err) {
        broadcast('ytdlp:error', { requestId, message: (err as Error).message });
      } finally {
        if (tempDir) {
  ...
  ipcMain.handle('ytdlp:cancel', async (_e, requestId: string) => {
    const entry = inflight.get(requestId);
    if (!entry) return;
    entry.canceled = true;
